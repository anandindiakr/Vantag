...
def _save_edge_preview(tenant_id: str, camera_id: str, b64: str) -> str | None:
    """Persist the latest live-preview JPEG from the edge agent under a fixed
    filename (``preview.jpg``) so it is overwritten on every push and never
    accumulates. Served by the cameras snapshot endpoint when no on-box
    pipeline is running (the normal SaaS/edge case).

    Returns the on-disk path written, or None on failure.
    """
    if not b64:
        return None
    try:
        ...
        cam_dir = _SNAPSHOTS_ROOT / str(tenant_id) / str(camera_id)
        cam_dir.mkdir(parents=True, exist_ok=True)
        (cam_dir / "preview.jpg").write_bytes(raw)
        return str(cam_dir / "preview.jpg")
    except Exception:  # noqa: BLE001
        return None
        ...
        "cameras": [
            {
                ...
                "rtsp_url": (c.get_rtsp_url() if hasattr(c, "get_rtsp_url") else c.rtsp_url) or "",
                "name": c.name,
                "location": c.location,
...
@edge_router.post("/preview")
async def ingest_preview(
    body: PreviewBody,
    agent: EdgeAgent = Depends(_verify_agent),
) -> dict:
    """Receive a periodic live-preview frame from the edge agent and cache it
    under ``snapshots/{tenant}/{camera}/preview.jpg`` so the dashboard's camera
    tiles can render a near-real-time still even though the RTSP stream itself
    never reaches the cloud (it runs entirely on the agent).
    """
    saved = _save_edge_preview(agent.tenant_id, body.camera_id, body.frame_b64)
