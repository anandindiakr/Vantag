...
async def submit_feedback(body: FeedbackBody) -> dict:
    """
    Store page-level feedback (helpful Y/N + optional comment).
    For now: logs to stdout and optionally emails if OpenAI key is absent.
    A future migration can write to a `feedback` DB table.
    """
    import logging
    logging.getLogger("vantag").info(
        "FEEDBACK page=%s helpful=%s topic=%r msg=%r",
        body.page, body.helpful, body.topic, body.message[:200],
    )
    # Best-effort email if configured
    email_key = os.getenv("SUPPORT_EMAIL_FROM", "")
    if email_key and not body.helpful and body.message:
        try:
            import smtplib, ssl
            ...
            if smtp_host and smtp_user:
                ...
                msg["Subject"] = f"[Vantag Feedback] {body.topic or body.page}"
                msg["From"] = smtp_user
