...
async def get_snapshot(
    ...
    # there is no local frame -> fall back to the periodic preview JPEG the
    # edge agent pushes to /api/edge/preview (cached under snapshots/).
    if _pipeline is None:
        from pathlib import Path as _P
        preview = (
            _P(__file__).resolve().parent.parent.parent
            / "snapshots" / str(user.get("tenant_id")) / str(camera_id) / "preview.jpg"
        )
        if preview.is_file():
            try:
