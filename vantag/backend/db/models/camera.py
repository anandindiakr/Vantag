...
class CameraConfig(Base):
    ...
    def get_rtsp_url(self) -> str | None:
        ...
        fernet = _get_fernet()
        if fernet is None:
            # No key configured -> we cannot recover the plaintext. Never hand
            # the encrypted "fernet:" token to the agent (it would fail to open
            # and spam errors); signal "no usable URL" instead.
            return None
        try:
            return fernet.decrypt(self.rtsp_url[len(_FERNET_PREFIX):].encode()).decode()
        except Exception:  # noqa: BLE001
            # Wrong/rotated key -> undecryptable. Return None so the camera
            # surfaces as "needs re-save" rather than feeding an encrypted blob.
            return None
