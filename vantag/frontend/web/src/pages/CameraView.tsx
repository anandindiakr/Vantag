...
export default function CameraView() {
  ...
  // The on-box MJPEG `/stream` endpoint only works when a local pipeline is
  // running. In the edge/SaaS deployment frames arrive from the Windows agent
  // and are served as a JPEG by `/cameras/{id}/snapshot`. An <img> tag cannot
  // send the JWT auth header, so we fetch the JPEG through the authenticated
  // axios instance as a blob and refresh it every few seconds.
  useEffect(() => {
    if (!cameraId) return;
    let cancelled = false;
    ...
    const fetchSnapshot = async () => {
      try {
        const res = await api.get(`/cameras/${cameraId}/snapshot`, {
          responseType: 'blob',
        });
        if (cancelled) return;
    ...
    fetchSnapshot();
    const id = window.setInterval(fetchSnapshot, 3000);
    return () => {
      cancelled = true;
          ...
          <div className="xl:col-span-2 space-y-4">
            ...
            >
              {cameras.length === 0 ? (
                ...
                <div className="flex flex-col items-center justify-center aspect-video bg-slate-900 gap-3 text-slate-500">
                  <Loader2 size={28} className="animate-spin" />
                  <p className="text-xs">
                    {snapshotError
                      ? 'Waiting for the edge agent to send a frame…'
